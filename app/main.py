...
st.set_page_config(page_title="ANE - PQRs", page_icon="📝", layout="wide")
...
def cargar_base_conocimiento():
    archivos_gemini = []
    rutas_pdfs = [
        "docs/CompilatoriaResANE105_2020_Mod_Marzo_2024_1.pdf",
        "docs/Resolucion_737_de_2022 _Modifica_Res_105_de_2020.pdf"
    ]
                    ...
                    try:
                        ...
                        for k, v in new_data.items():
                            if v and k in st.session_state.doc_data:
                                st.session_state.doc_data[k] = str(v)
...
with st.sidebar:
    ...
    trato   = st.text_input("Trato (Señor/Señora)", value=st.session_state.doc_data["TRATO"])
    nombre  = st.text_input("Nombre",          value=st.session_state.doc_data["NOMBRE"])
    correo  = st.text_input("Correo electrónico", value=st.session_state.doc_data["CORREO"])
    ...
    asunto  = st.text_area("Asunto",           value=st.session_state.doc_data["ASUNTO"], height=68)
    saludo  = st.text_input("Saludo Inicial",  value=st.session_state.doc_data["SALUDO_INICIAL"])
    cuerpo  = st.text_area("Cuerpo",           value=st.session_state.doc_data["CUERPO"], height=300)
    ...
    if st.button("📄 Generar y Descargar Carta", type="primary", use_container_width=True):
        ...
        try:
            doc = DocxTemplate("templates/plantillaPQR.docx")
            datos_render = st.session_state.doc_data.copy()
            datos_render["CUERPO"] = procesar_cuerpo_a_word(doc, st.session_state.doc_data["CUERPO"])
